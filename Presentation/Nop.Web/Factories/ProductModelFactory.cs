    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            if (product.HasEMIAvailable ?? false)
            {
                ...
                model.EMI = new EMIModel()
                {
                    Rate = product.InterestRate ?? 0,
                    AvailableMonth = availableMonth,
                    Amount = product.Price - ((product.Price * product.DownPaymentPercent ?? 0) / 100),
                };
                model.EMI.MonthlyRepayment = _emiService.GetScheduleDetails(model.EMI.Amount, model.EMI.Rate, 12).Total;
